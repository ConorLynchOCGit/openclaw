...
5. For workspace project documents, pass `scope: "operator_workspace"` explicitly. Bare `docs/...` paths usually mean product repo docs; workspace continuity docs are `operator_workspace docs/projects/...` or `scope: "operator_workspace", path: "docs/projects/..."`.
6. Keep reads and lists targeted. Do not broad-scan the workspace or host mirrors.
7. For edits, require `OPENCLAW_HOST_OPERATOR_WRITE_ENABLED=true`.
8. For exec, require `OPENCLAW_HOST_OPERATOR_EXEC_ENABLED=true`; exec is only allowed in `live_repo` scope.
9. Preserve the audit id/path in your summary when host-operator tooling is used.
...
- `projects/**` legacy/live workspace project docs
- `runbooks/**`
- `memory/**`
- `AGENTS.md`
- `HEARTBEAT.md`
...
The live repo scope may edit, when the write kill switch is enabled:

- `docs/projects/**`
- `docs/agents/**`
- `.agents/skills/**`
- `skills/**`
- source, UI, script, and test files under the live repo

Main should use live repo scope for implementation-owned docs and agent pack docs, including Web Researcher files. Main should use operator workspace scope for continuity/project-management docs under `/root/.openclaw/workspace/docs/projects` and `/root/.openclaw/workspace/projects`.
