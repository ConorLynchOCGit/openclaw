...
- For file/source targets, use `node scripts/test-projects.mjs <targets...>`; it is the sole owner/config router. Use `scripts/run-vitest.mjs` only when a repository-owned command already specifies the exact config. Never use raw `vitest` as a routing substitute.
