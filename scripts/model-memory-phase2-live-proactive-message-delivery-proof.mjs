...
function assertNoProhibitedContent(value) {
  ...
  for (const parts of [
    ...
    if (serialized.includes(parts.join(""))) {
      throw new Error(
        `live proactive delivery proof contains prohibited marker: ${parts.join("")}`,
      );
    }
  }
}
...
async function waitForVisibleDelivery(harness, expectedText, timeoutMs = 30_000) {
  ...
  throw new Error(
    `timed out waiting for live proactive delivery text; tail=${lastTail.slice(0, 160)}`,
  );
}
...
async function main() {
  ...
  const outputDir = path.join(
    root,
    ".artifacts/model-memory/phase2-live-proactive-message-delivery-proof",
    stamp,
  );
  await mkdir(outputDir, { recursive: true });

  const marker = `PHASE2-LIVE-PROACTIVE-MESSAGE-DELIVERY-${stamp}`;
  const sessionKey =
    process.env.MODEL_MEMORY_PHASE2_LIVE_PROACTIVE_DELIVERY_SESSION ?? DEFAULT_MAIN_SESSION_ALIAS;
  ...
  try {
    ...
    proofTurn = await harness.sendPrompt(
      [
        "Operator Phase 2 live proactive message delivery proof.",
        `Proof marker: ${marker}-LIVE-SEAM.`,
        "Prepare to observe a bounded live proactive message delivered only after explicit approval.",
      ].join("\n"),
    ...
    const outsideScopeReport = await buildPhase2LiveProactiveMessageDeliveryReport({
      proofMarker: `${marker}-OUTSIDE-SCOPE`,
      now: new Date(),
      controlledReport: await buildPhase2ControlledUserFacingProactivityReport({
        proofMarker: `${marker}-OUTSIDE-SCOPE-CONTROLLED`,
        now: new Date(),
        approvedScope,
    ...
    if (outsideScopeReport.decision !== "blocked_scope") {
      throw new Error(`outside-scope delivery block failed: ${outsideScopeReport.decision}`);
    }

    const missingSendApprovalReport = await buildPhase2LiveProactiveMessageDeliveryReport({
      proofMarker: `${marker}-MISSING-SEND-APPROVAL`,
      now: new Date(),
      controlledReport: await buildPhase2ControlledUserFacingProactivityReport({
        proofMarker: `${marker}-MISSING-SEND-APPROVAL-CONTROLLED`,
        now: new Date(),
        approvedScope,
        requestScope,
    ...
    if (missingSendApprovalReport.decision !== "blocked_missing_send_approval") {
      throw new Error(`missing send approval block failed: ${missingSendApprovalReport.decision}`);
    }
    ...
    if (blockedClassReport.decision !== "blocked_message_class") {
      throw new Error(`blocked class delivery block failed: ${blockedClassReport.decision}`);
    }

    const rollbackReport = await buildPhase2LiveProactiveMessageDeliveryReport({
    ...
    if (rollbackReport.decision !== "blocked_rollback") {
      throw new Error(`rollback delivery block failed: ${rollbackReport.decision}`);
    }
    ...
    report.uiEvidence = {
      sessionKey,
      proofMarker: marker,
      liveDeliveryRunId: proofTurn?.runId ?? null,
      ...
      JSON.stringify(
        {
          ...
          adapterMessageId: report.deliveryResult.adapterMessageId,
