...
const DEFAULT_MODEL_ID = "openai-codex/gpt-5.4";
const PROMPTS = [
...
const PRIMARY_BAD_SNIPPETS = [
  ...
  "It sets the default packaging strategy",
  "It sets the default",
  "Source: chat://",
  "chat://",
  "gateway://",
    ...
    return {
      ...
      handoff: handoffMessage
        ? {
            ...
            containsBadSnippet:
              /\b(?:Question worth asking before|Turn Turn|Already recurring|Build the bounded request with|It sets the default)\b/iu.test(
                handoffMessage,
              ),
          }
        : null,
...
async function waitForPresentationState(page, sessionKey, predicate, timeoutMs = 120_000) {
...
function toMarkdown(summary) {
  return [
    "# Phase 2 Model-Authored Proactivity Briefs Proof",
    "",
    `- generatedAt: ${summary.generatedAt}`,
    `- sessionKey: ${summary.sessionKey}`,
    `- configuredModelTarget: ${summary.configuredModelTarget}`,
    `- uiAuthWorks: ${summary.evidence.uiAuthWorks}`,
    `- modelAuthoredBriefVisible: ${summary.evidence.modelAuthoredBriefVisible}`,
    `- gpt54FastObserved: ${summary.evidence.gpt54FastObserved}`,
    `- primaryCardsClean: ${summary.evidence.primaryCardsClean}`,
    `- genericFillerHidden: ${summary.evidence.genericFillerHidden}`,
...
async function writeArtifacts(outputDir, payload) {
  ...
  const jsonPath = path.join(outputDir, "model-authored-proactivity-briefs-proof.json");
  const markdownPath = path.join(outputDir, "model-authored-proactivity-briefs-proof.md");
...
async function main() {
  ...
  const outputDir = path.join(
    root,
    ".artifacts/model-memory/phase2-model-authored-proactivity-briefs-proof",
    stamp,
  );
  await mkdir(outputDir, { recursive: true });
  const origin = process.env.OPENCLAW_TAILNET_ORIGIN || DEFAULT_TAILNET_ORIGIN;
  const sessionKey =
    process.env.MODEL_MEMORY_PHASE2_MODEL_AUTHORED_BRIEFS_SESSION ?? DEFAULT_SESSION_KEY;
  const configuredModelTarget =
    process.env.MODEL_MEMORY_PHASE2_MODEL_AUTHORED_BRIEFS_MODEL || DEFAULT_MODEL_ID;
  const evidence = {
    uiAuthWorks: false,
    modelAuthoredBriefVisible: false,
    gpt54FastObserved: false,
    skillCandidateBriefVisible: false,
    primaryCardsClean: false,
    genericFillerHidden: false,
    titleNextStepRepetitionHidden: false,
  ...
  try {
    ...
    if (process.env.MODEL_MEMORY_PHASE2_MODEL_AUTHORED_BRIEFS_SKIP_RESET !== "1") {
      ...
      (state) =>
        state.queueItems.some(
          (item) =>
            ...
            item.briefAuthorship?.source === "model" &&
            item.briefQuality?.status !== "demote",
        ),
      150_000,
    );
    if (!finalState) {
      throw new Error("model-authored proactivity brief state was not readable");
    }

    const modelItems = finalState.queueItems.filter(
      (item) => item.briefAuthorship?.source === "model",
    );
    summary.modelIds = [
      ...new Set(modelItems.map((item) => item.briefAuthorship?.modelId).filter(Boolean)),
    ];
    const skillItem =
      modelItems.find(
        (item) =>
    ...
    if (!skillItem) {
      throw new Error("no live skill candidate with a model-authored brief appeared");
    }
    ...
    evidence.modelAuthoredBriefVisible = modelItems.some(
      (item) => item.briefQuality?.status !== "demote",
    );
    evidence.gpt54FastObserved = summary.modelIds.some((modelId) =>
      String(modelId).toLowerCase().includes("gpt-5.4"),
    );
    ...
    evidence.genericFillerHidden = allPrimaryCards.every(
      (card) =>
        !/Turns a recent idea into a bounded next step|without digging through the inbox|Already recurring|Build the bounded request with|It sets the default/iu.test(
          card.primaryText,
        ),
    );
    evidence.titleNextStepRepetitionHidden = allPrimaryCards.every(
      (card) =>
        !/It sets the default packaging strategy.*It sets the default packaging strategy/isu.test(
          card.primaryText,
        ),
    );
    ...
    evidence.canonicalIdsShared =
      ...
      (finalState.heartbeatCards.length === 0 ||
        finalState.heartbeatCards.some(
          (card) => card.skillCandidateId === skillItem.skillCandidateId,
        ));
    ...
    evidence.handoffUsesBrief =
      ...
      finalState.handoff.containsBadSnippet === false;
    ...
    summary.ok =
      ...
      evidence.modelAuthoredBriefVisible &&
      evidence.gpt54FastObserved &&
      evidence.skillCandidateBriefVisible &&
      evidence.primaryCardsClean &&
      evidence.genericFillerHidden &&
      evidence.titleNextStepRepetitionHidden &&
      evidence.malformedReversePromptDemoted &&
      evidence.goodReversePromptVisibleOrNoMalformed &&
      evidence.canonicalIdsShared &&
      evidence.heartbeatClean &&
