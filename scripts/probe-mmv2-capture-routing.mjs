...
const CASES = [
  {
    id: "routing-preference-claim",
    text: "I prefer concise answers.",
    expectedRoutes: [{ match: "I prefer concise answers.", route: "atomic_candidate" }],
  },
  {
    id: "routing-procedure-composite",
    text: "Release checklist:\n1. Run the test suite.\n2. Ship the build.",
    expectedRoutes: [{ matchShape: "numbered_list_block", route: "composite_candidate" }],
  },
];
  ...
  for (const result of report.results) {
    ...
    if (result.routingDecisions.length > 0) {
      ...
      for (const decision of result.routingDecisions) {
        lines.push(
          `  - ${decision.segment_id}: ${decision.route} | evidence=${JSON.stringify(decision.evidence_quote)}`,
        );
      }
    }
    lines.push("");
...
async function main() {
  ...
  const [
    ...
    tsImport("../extensions/model-memory/src/mmv2/capture-routing.ts", {
      parentURL: import.meta.url,
    }),
  ...
  for (const mode of modes) {
    for (const testCase of CASES) {
      ...
      try {
        const routing = await captureRoutingExports.routeCaptureCandidates({
          rawEvent,
          segmented,
          sourceKind: "document",
          sourceId: envelope.source.id,
        ...
        const expectationOk = testCase.expectedRoutes.every((expected) => {
          const matchedSegment = segmented.segments.find((segment) => {
            if (expected.match) {
              return segment.text.includes(expected.match);
            }
            if (expected.matchShape) {
              return segment.detected_shape === expected.matchShape;
            }
            return false;
          });
          if (!matchedSegment) {
          ...
          const matchedDecision = routing.routing_decisions.find(
            (decision) => decision.segment_id === matchedSegment.segment_id,
          );
          return matchedDecision?.route === expected.route;
        });
        ...
        results.push({
          ...
          traces: traceSlice,
          routingDecisions: routing.routing_decisions,
  ...
  const artifacts = await writeArtifacts(
    args.outputRoot ?? path.join(repoRoot, ".artifacts/model-memory/mmv2/routing-probe"),
    report,
  );
