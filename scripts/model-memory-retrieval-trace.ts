...
import {
  renderModelMemoryRetrievalTraceMarkdown,
  runModelMemoryRetrievalTrace,
} from "../src/agents/model-memory.retrieval-trace.js";
import {
...
const DEFAULT_PROBE_ID = process.env.MODEL_MEMORY_RETRIEVAL_PROBE_ID?.trim() || "probe-live-tests";
...
async function main() {
  ...
  try {
    ...
    const jsonPath = path.join(evidenceDir, `retrieval-trace-${slug}.json`);
    const markdownPath = path.join(evidenceDir, `retrieval-trace-${slug}.md`);
    await writeFile(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
    await writeFile(markdownPath, `${renderModelMemoryRetrievalTraceMarkdown(report)}\n`, "utf8");
