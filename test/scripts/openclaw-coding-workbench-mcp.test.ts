...
describe("openclaw-coding-workbench MCP helpers", () => {
  ...
  it("stops TypeScript dependency traversal at the package-owned file budget", async () => {
    const repo = await makeRepo();
    for (let index = 0; index < 25; index += 1) {
      const source =
        index === 24
          ? `export const value${index} = ${index};\n`
          : `import { value${index + 1} } from "./chain-${index + 1}.js";\nexport const value${index} = value${index + 1};\n`;
      await fs.writeFile(path.join(repo, "src", `chain-${index}.ts`), source, "utf8");
    }
    const workbench = await loadWorkbench();

    const definition = await workbench.lspDefinitionTypescript(
      { file: "src/chain-0.ts", line: 2, character: 23 },
      optionsFor(repo),
    );
    ...
    expect(definition).toMatchObject({
      ...
      definitions: [expect.objectContaining({ path: "src/chain-1.ts", line: 2 })],
    });
  });
