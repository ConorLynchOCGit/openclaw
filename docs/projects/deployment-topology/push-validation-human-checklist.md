...
| Operator-review session presence  | Open the session selector after running the daily and weekly operator-review jobs                                                                                                                              | Named sessions `Daily Operator Review` and `Weekly Operator Review` are visible and stable                                      | The operator-review sessions are missing from the selector or appear as raw ids                                    | `before_push`          |
...
| Live progress in chat             | In Main, start a deliberately long-running task and watch the live chat transcript                                                                                                                             | Chat shows bounded `Queued:` or `Working:` progress before the task finishes                                                    | Chat looks stalled until the final answer only                                                                     | `before_push`          |
| Detached replay in chat           | Start a long-running task, let it detach or background, then return to the session and send a small follow-up                                                                                                  | The next turn replays one bounded current-state or terminal summary from runtime-owned replay state                             | No replay, stale replay, or duplicate replay on every follow-up                                                    | `before_push`          |
...
| Daily continuity artifact         | Trigger a continuity-producing session and inspect today’s `memory/YYYY-MM-DD.md`                                                                                                                              | The canonical daily memory artifact is created or updated by the retained continuity hook                                       | No daily memory artifact update or obvious continuity-hook regression                                              | `before_push`          |
| Skill-vetting surface             | Ask Main or run the documented skill-vetting path for a bounded external skill review                                                                                                                          | The system can explain or exercise the canonical `install` / `inspire` / `reject` review posture                                | No discoverable skill-vetting lane or external skills treated as blind installs                                    | `before_push`          |
| Root gate dry-run visibility      | Run `pnpm exec turbo run check:root:all --filter=openclaw --dry=json`, `pnpm exec turbo run test:root:all --filter=openclaw --dry=json`, and `pnpm exec turbo run build:root:all --filter=openclaw --dry=json` | Each command shows explicit root-stage tasks plus dependencies instead of one opaque root body                                  | Dry-run still collapses to one monolithic root command                                                             | `before_commit`        |
...
- re-check detached/background replay after at least one real long-running task
  completes outside the active turn
