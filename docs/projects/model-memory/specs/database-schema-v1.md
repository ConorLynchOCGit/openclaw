...
### `memory_support_items`

Purpose:

- attach one or more source-backed supports to a durable memory object
...
- unique on `(memory_object_id, support_fingerprint)`

Indexes:

- index on `memory_object_id`
- index on `source_window_id`
- index on `support_kind`
- index on `counts_for_reinforcement`
...
- `support_item_id -> model_memory.memory_support_items(id)`

Indexes:

- index on `source_window_id`
- index on `memory_object_id`
- index on `support_item_id`
- index on `decision`
- index on `created_at`
