...
Generated at: 2026-04-18T16:32:32.901Z
...
- total windows: 10
- candidate counts by type: {"reference":6,"rule":140,"procedure":4,"fact":7}
- canonicalized counts by kind: {"reference":3,"rule":64,"fact":3}
- validation rejects: 0
- write decisions: {"write":69,"attach_support":1}
- adjudication outcomes: {"no_candidates":63,"local_conflict_hold_ambiguous":1,"direct_distinct":5}

| document                                                                              | windows | pass1 candidates                     | accepted kinds                      | accepted classes              | rejects | writes                            | active delta                         | memory projection       |
| ------------------------------------------------------------------------------------- | ------: | ------------------------------------ | ----------------------------------- | ----------------------------- | ------: | --------------------------------- | ------------------------------------ | ----------------------- |
| `docs/system/memory.md`                                                               |       1 | `{"reference":2,"rule":13}`          | `{"reference":1,"rule":6}`          | `{"reference":1,"project":6}` |       0 | `{"write":7}`                     | `{"reference":1,"rule":5}`           | `MEMORY.md (2 bullets)` |
| `docs/system/deployment.md`                                                           |       1 | `{"rule":22}`                        | `{"rule":11}`                       | `{"project":11}`              |       0 | `{"write":10,"attach_support":1}` | `{"reference":0,"rule":10}`          | `MEMORY.md (2 bullets)` |
| `docs/projects/model-memory/specs/architecture-overview.md`                           |       1 | `{"rule":14}`                        | `{"rule":7}`                        | `{"user":1,"project":6}`      |       0 | `{"write":7}`                     | `{"reference":0,"rule":7}`           | `MEMORY.md (2 bullets)` |
| `docs/projects/model-memory/specs/kind-primary-schema-migration.md`                   |       1 | `{"rule":8,"procedure":4}`           | `{"rule":3}`                        | `{"project":3}`               |       0 | `{"write":3}`                     | `{"reference":0,"rule":3}`           | `MEMORY.md (2 bullets)` |
| `docs/projects/model-memory/specs/document-read-and-ingest-arbitration.md`            |       1 | `{"rule":18,"fact":2}`               | `{"rule":8,"fact":1}`               | `{"project":8,"user":1}`      |       0 | `{"write":9}`                     | `{"reference":0,"rule":8,"fact":1}`  | `MEMORY.md (2 bullets)` |
| `docs/projects/workspace-topology/runtime-project-surfaces.md`                        |       1 | `{"rule":7}`                         | `{}`                                | `{}`                          |       0 | `{}`                              | `{"reference":0,"rule":0,"fact":0}`  | `MEMORY.md (2 bullets)` |
| `docs/projects/deployment-topology/github-automation.md`                              |       1 | `{"fact":4,"rule":4,"reference":2}`  | `{"fact":2,"rule":2,"reference":1}` | `{"project":4,"reference":1}` |       0 | `{"write":5}`                     | `{"reference":1,"rule":2,"fact":2}`  | `MEMORY.md (4 bullets)` |
| `docs/projects/skills-system/skill-vetting/specs/operator-vetting-report-contract.md` |       1 | `{"rule":24}`                        | `{"rule":12}`                       | `{"project":12}`              |       0 | `{"write":12}`                    | `{"reference":0,"rule":12,"fact":0}` | `MEMORY.md (4 bullets)` |
| `docs/projects/web-stack/web_research_delegation_spec.md`                             |       1 | `{"fact":1,"reference":2,"rule":14}` | `{"reference":1,"rule":7}`          | `{"reference":1,"project":7}` |       0 | `{"write":8}`                     | `{"reference":1,"rule":7,"fact":0}`  | `MEMORY.md (4 bullets)` |
| `docs/projects/intake-routing/ROUTING_IMPLEMENTATION_CHECKLIST.md`                    |       1 | `{"rule":16}`                        | `{"rule":8}`                        | `{"project":8}`               |       0 | `{"write":8}`                     | `{"reference":0,"rule":8,"fact":0}`  | `MEMORY.md (4 bullets)` |
