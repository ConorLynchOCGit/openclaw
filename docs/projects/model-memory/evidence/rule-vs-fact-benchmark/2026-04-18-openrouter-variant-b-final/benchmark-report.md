...
Generated at: 2026-04-18T16:21:10.500Z
...
- total windows: 10
- candidate counts by type: {"rule":64,"procedure":1,"fact":6}
- canonicalized counts by kind: {"fact":8,"rule":61,"procedure":1}
- validation rejects: 0
- write decisions: {"write":70}
- adjudication outcomes: {"no_candidates":63,"local_conflict_hold_ambiguous":5,"direct_distinct":2}

| document                                                                              | windows | pass1 candidates           | accepted kinds                      | accepted classes             | rejects | writes         | active delta                         | memory projection       |
| ------------------------------------------------------------------------------------- | ------: | -------------------------- | ----------------------------------- | ---------------------------- | ------: | -------------- | ------------------------------------ | ----------------------- |
| `docs/system/memory.md`                                                               |       1 | `{"rule":6}`               | `{"fact":1,"rule":5}`               | `{"project":6}`              |       0 | `{"write":6}`  | `{"fact":1,"rule":5}`                | `MEMORY.md (2 bullets)` |
| `docs/system/deployment.md`                                                           |       1 | `{"rule":9}`               | `{"fact":1,"rule":8}`               | `{"project":9}`              |       0 | `{"write":9}`  | `{"fact":1,"rule":7}`                | `MEMORY.md (3 bullets)` |
| `docs/projects/model-memory/specs/architecture-overview.md`                           |       1 | `{"rule":5}`               | `{"rule":5}`                        | `{"project":5}`              |       0 | `{"write":5}`  | `{"fact":0,"rule":4}`                | `MEMORY.md (3 bullets)` |
| `docs/projects/model-memory/specs/kind-primary-schema-migration.md`                   |       1 | `{"rule":4,"procedure":1}` | `{"fact":1,"rule":3,"procedure":1}` | `{"project":4,"feedback":1}` |       0 | `{"write":5}`  | `{"fact":1,"rule":3,"procedure":1}`  | `MEMORY.md (4 bullets)` |
| `docs/projects/model-memory/specs/document-read-and-ingest-arbitration.md`            |       1 | `{"rule":10,"fact":1}`     | `{"rule":10}`                       | `{"project":10}`             |       0 | `{"write":10}` | `{"fact":0,"rule":10,"procedure":0}` | `MEMORY.md (4 bullets)` |
| `docs/projects/workspace-topology/runtime-project-surfaces.md`                        |       1 | `{"fact":2,"rule":4}`      | `{"fact":2,"rule":4}`               | `{"project":6}`              |       0 | `{"write":6}`  | `{"fact":2,"rule":3,"procedure":0}`  | `MEMORY.md (6 bullets)` |
| `docs/projects/deployment-topology/github-automation.md`                              |       1 | `{"fact":2,"rule":3}`      | `{"fact":2,"rule":3}`               | `{"project":5}`              |       0 | `{"write":5}`  | `{"fact":2,"rule":2,"procedure":0}`  | `MEMORY.md (8 bullets)` |
| `docs/projects/skills-system/skill-vetting/specs/operator-vetting-report-contract.md` |       1 | `{"rule":11}`              | `{"rule":11}`                       | `{"project":11}`             |       0 | `{"write":11}` | `{"fact":0,"rule":10,"procedure":0}` | `MEMORY.md (8 bullets)` |
| `docs/projects/web-stack/web_research_delegation_spec.md`                             |       1 | `{"fact":1,"rule":5}`      | `{"fact":1,"rule":5}`               | `{"project":6}`              |       0 | `{"write":6}`  | `{"fact":1,"rule":5,"procedure":0}`  | `MEMORY.md (9 bullets)` |
| `docs/projects/intake-routing/ROUTING_IMPLEMENTATION_CHECKLIST.md`                    |       1 | `{"rule":7}`               | `{"rule":7}`                        | `{"project":7}`              |       0 | `{"write":7}`  | `{"fact":0,"rule":7,"procedure":0}`  | `MEMORY.md (9 bullets)` |
