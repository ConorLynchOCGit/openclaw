...
Observed rerun result on the explicit small-model lane:
...
- rerunning the same Tier 1 cases still creates new writes instead of clean
  duplicate collapse
- no Tier 1 document is admitted into audited proof yet

This means the inventory itself remains valid, but the current live
large-document extraction path is still not yet stable enough for proof
admission.
