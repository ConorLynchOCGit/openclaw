...
- all six Tier 1 documents now persist canonical objects with structured
  provenance
- however, repeated runs still create more fresh active growth than the project
  can currently treat as bounded semantic convergence, so the large-source proof
  set is still not ready for admission

Current blocker classes still seen in Tier 1 evidence:

- bounded-convergence failure:
  - the same Tier 1 source still produces too much fresh active growth on
    rerun instead of recurring core claims plus bounded active expansion
- over-capture / source-sensitivity:
  ...
  - `docs/projects/model-memory/proof-corpus-plan.md` remains
    ...
    plan itself
- proof admission remains blocked until at least one real-source case is
  structurally valid, semantically convergent enough in aggregate, and bounded
  in active growth
...
- the next pass must improve bounded semantic convergence and duplicate
  identity quality in aggregate, then explicitly adjudicate whether any Tier 1
  case is honest enough for proof admission

## Representative pass-1 comparison status

Default run posture remains nano/nano:

- pass 1 = `openrouter/openai/gpt-5.4-nano`
- pass 2 = `openrouter/openai/gpt-5.4-nano`

The mini lane below was a bounded comparison run only, not the default lane.
...
- pass-1-on-mini improved coarse representative outcomes:
  ...
  - `docs/gateway/protocol.md` avoided the nano reject path and held at
    `9 / 8` instead of `0 / 6`
- the candidate sets still drifted materially under the same seed, which is
  retained here as historical comparison evidence rather than the current proof
  bar
...
- therefore:
  ...
  - the next honest blocker remains bounded semantic convergence and active-set
    control, not exact candidate-set stability

## Real-source proof-admission standard

Real-source proof admission does not require exact rerun candidate overlap or
exact rerun object-set equality.

Those properties are not the current bar because they are not realistic on the
accepted nano/nano model lane and treating them as hard gates would encode a
known model limitation as a project blocker.

A real-source case is eligible for audited proof only if:

- it remains structurally valid across repeated runs
- provenance remains valid and source-window-bounded
- it does not rely on local semantic rescue to pass
- the same core durable claims recur often enough across repeated runs to trust
  the case, even if the exact candidate set drifts
- repeated runs do not create materially unbounded active-object growth
- drift is mostly absorbed by omission, support attachment, bounded
  containment, or otherwise limited active growth
- provisional and `conflict_hold` objects do not leak into default runtime
  reads
- the source itself is honest for proof and not bootstrap-sensitive or
  proof-fixture-contaminating in a way that would make admission dishonest

Working interpretation:

- exact rerun equality is not required
- candidate-set overlap is not required
- bounded semantic convergence is required
- bounded active-set growth is required
- source honesty is required
