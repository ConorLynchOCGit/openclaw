...
The authenticated browser prompt harness now exists and has already rerun the
model-memory and ingest-adjacent operator rows.

Current state from that pass:
...
Use this document for the next targeted soak rerun or fix validation, not to
reconstruct whether those rows have already been exercised.
...
| prompt_ingest          | durable token capture              | In a fresh Main session, choose a fresh unique token for this run and say: `For the rest of this test phase, remember this exact token for later retrieval: <unique-token>.` | The runtime attempts or performs ordinary-turn capture                                                    | session log, later DB query, later retrieval prompt           | no capture signal or no later retrieval path                                               | `no`                             |
...
| prompt_ingest          | later token retrieval              | In a later fresh Main session, ask: `What exact token did I ask you to remember during this test phase? Return the token only.`                                              | Retrieves the same fresh unique token from this run through the memory path                               | session transcript, retrieval trace, DB checks                | cannot retrieve the token or answers without memory evidence                               | `no`                             |
...
- Tailnet authenticated browser bootstrap is now a solved prerequisite, not the
  remaining blocker for this soak. The remaining blockers are active ingest
  availability and the live prompt/replay execution itself.
