...
<task id="agent:execution-context-scout:subagent:..." state="completed">
  ...
  <task_result>
    ...
    Parent decision required: update todo, then choose one:
    start minimal edit / ask exact follow-up / ask map pass / blocked.
  </task_result>
</task>
```
...
<task id="..." state="error">
  <task_error>
    ...typed failure...
    Parent decision required: update todo, then choose one:
    ask narrower context / retry after repair / blocked.
  </task_error>
</task>
```

The footer is parent-visible task-result text, not durable event semantics.
...
- never edits;
- never returns full files unless a bounded full file is explicitly tiny and useful;
...
Return:
  map plus minimum edit-start windows, not full files.

Useful result means:
  enough context for Kimi to either start a bounded edit or ask one exact follow-up.
...
Edit-Start Recommendation
  enough_for_minimal_edit | need_exact_window | need_map_pass | blocked_by_missing_source
...
| 4. Context scout skill update using OpenCode `explore` shape                       | Complete                    | `skills/execution-context-scout/SKILL.md` now includes `quick`/`medium`/`very thorough`, bounded-window behavior for full-file asks, and explicit `edit_start_recommendation` values.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 5. Kimi skill update for minimum viable edit context                               | Complete                    | `skills/execution-node-workflow/SKILL.md` now consumes `enough_for_minimal_edit`, `need_exact_window`, `need_map_pass`, and `blocked_by_missing_source` and maps each to the next todo/task/edit/block action.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 6. Task result decision checkpoint wording                                         | Complete                    | Context and validation scout task results include parent-visible decision footers; focused tests passed: `pnpm test:file src/agents/tools/native-task-tool.test.ts -- -t "runs foreground and returns child result text"` and `pnpm test:file src/agents/tools/native-task-tool.test.ts -- -t "appends the validation parent-decision footer"`.                                                                                                                                                                                                                                                                                                                                                              |
...
| 14. OpenCode-style task result envelope                                            | Complete                    | Native task parent-visible output now uses an OpenCode-style `<task>` envelope with `<task_result>` / `<task_error>` and keeps the decision footer inside model-visible tool-result text, not durable event semantics. Focused test passed: `pnpm test:file src/agents/tools/native-task-tool.test.ts -- -t "projects structured oversized child output                                                                                                                                                                                                                                                                                                                                                      | projects unstructured oversized child output                                                                                          | appends the validation parent-decision footer"`. |
