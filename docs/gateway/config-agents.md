...
- `executionWorkspace`: optional trusted authorization ceiling for task-requested native hidden sub-agent workspaces. `{ type: "loaded-source", access: "inspect" }` permits an exact-generation checkout without repository setup; `access: "modify"` permits trusted isolated setup before inference. Configuration alone does not switch roots. The task requests the semantic checkout, OpenClaw derives the source commit from the executing package, and native managed-worktree/session lifecycle owns creation and cleanup. The configured agent workspace remains the bootstrap and capability authority while task file tools use the checkout `cwd`.
