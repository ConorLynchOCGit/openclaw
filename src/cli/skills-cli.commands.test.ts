...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    buildPluginRegistrySnapshotReportMock: vi.fn(),
...
vi.mock("../runtime.js", () => ({
  ...
  writeRuntimeJson: (runtime: typeof mocks.defaultRuntime, value: unknown) =>
...
vi.mock("../agents/agent-scope.js", () => ({
  listAgentEntries: (config: { agents?: { list?: Array<{ id?: string }> } }) =>
  ...
  listAgentIds: (config: { agents?: { list?: Array<{ id?: string }> } }) =>
    Array.isArray(config.agents?.list) && config.agents.list.length > 0
      ? config.agents.list.map((entry) => entry.id ?? "main")
      : ["main"],
...
vi.mock("../plugins/status-snapshot.js", () => ({
  buildPluginRegistrySnapshotReport: (...args: unknown[]) =>
    mocks.buildPluginRegistrySnapshotReportMock(...args),
}));
...
describe("skills cli commands", () => {
  ...
  it("runs read-only lifecycle audit with advisory JSON", async () => {
    await runCommand(["skills", "audit-lifecycle", "--agent", "main", "--json"]);

    expect(buildWorkspaceSkillStatusMock).toHaveBeenCalledWith(
      "/tmp/workspace",
      expect.objectContaining({ agentId: "main" }),
    );
    expect(buildPluginRegistrySnapshotReportMock).toHaveBeenCalledWith({
      config: {},
      workspaceDir: "/tmp/workspace",
    });
