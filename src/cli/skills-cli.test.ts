...
function createMockReport(skills: SkillStatusEntry[]): SkillStatusReport {
  return {
    ...
    activatableSkillNames: skills.filter((skill) => skill.activatable).map((skill) => skill.name),
    modelVisibleSkillNames: skills.filter((skill) => skill.modelVisible).map((skill) => skill.name),
