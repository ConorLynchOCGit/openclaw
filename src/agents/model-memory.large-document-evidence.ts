...
import {
  ...
  MemoryKindSchema,
  createExecutorBackedSemanticInterpreter,
  ingestDocumentLive,
...
function summarizeObject(record: ModelMemoryObjectRecord): LargeDocumentObjectSummary {
  const firstSpan = record.provenance[0];
...
function allProvenanceValid(records: ModelMemoryObjectRecord[]): boolean {
  return records.every(
    (record) =>
      record.provenance.length > 0 &&
      record.provenance.every((span) => Array.isArray(span.headingPath)),
  );
}
...
function buildDuplicateFindings(
  ...
  const findings: string[] = [];
  if ((firstRun.writeDecisionCounts.dedupe ?? 0) > 0) {
    findings.push("intra_document_duplicate_collapse_observed");
  }
  if (
    firstRun.capturedObjectCount > 0 &&
    (secondRun.writeDecisionCounts.dedupe ?? 0) === firstRun.capturedObjectCount
  ) {
    findings.push("full_rerun_dedupe_stability");
  }
...
async function summarizeLatestState(
  ...
  const objectsForSource = allObjects.filter((record) => windowIds.has(record.sourceWindowId));
  ...
  return {
    ...
    objectSummaries: objectsForSource.map(summarizeObject),
    ...
    provenanceValid: allProvenanceValid(objectsForSource),
...
export async function executeLargeDocumentEvidencePhase(input: {
  ...
  const interpreter = new RecordingSemanticInterpreter(
    createExecutorBackedSemanticInterpreter(executor),
  );
