...
function buildAnnounceReplyInstruction(params: {
  ...
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for parent review. Use the child result, bounded context, and refs as evidence for the current task; inspect refs when they could change the answer, then continue or answer based on your own judgment. Keep this internal context private (don't mention system/log/stats/session details or announce type). Reply ONLY: ${SILENT_REPLY_TOKEN} when no user-facing update is needed.`;
  }
