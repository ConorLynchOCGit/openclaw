...
import type {
  ...
  SessionToolChoiceSnapshot,
} from "../../config/sessions/types.js";
import { resolveMainMemoryRoutingDecision as resolveMainMemoryRoutingDecisionBase } from "../main-memory-routing.js";
...
export { resolveMainMemoryRoutingDecision } from "../main-memory-routing.js";

type MainMemoryRoutingDiagnostics = SessionMainMemoryRoutingReport;
const MAIN_MEMORY_ROUTING_CAPTURED = Symbol("mainMemoryRoutingCaptured");
type MutableMainMemoryRoutingDiagnostics = MainMemoryRoutingDiagnostics & {
  [MAIN_MEMORY_ROUTING_CAPTURED]?: boolean;
};
...
function normalizeToolChoiceSnapshot(value: unknown): SessionToolChoiceSnapshot {
  if (value == null) {
    return null;
  }
  ...
  const candidate = value as { type?: unknown; name?: unknown; function?: unknown };
  const normalized: { type?: string; name?: string } = {};
  ...
  if (typeof candidate.name === "string" && candidate.name.trim()) {
    ...
    ((candidate.function as { name?: string }).name?.trim()?.length ?? 0) > 0
  ) {
    normalized.name = (candidate.function as { name: string }).name;
  }
...
function updateMainMemoryRoutingDiagnostics(
  diagnostics: MainMemoryRoutingDiagnostics | undefined,
  updates: Partial<MainMemoryRoutingDiagnostics>,
): void {
  if (!diagnostics) {
    return;
  }
...
function hasCapturedMainMemoryRoutingDiagnostics(
  diagnostics: MainMemoryRoutingDiagnostics | undefined,
): diagnostics is MutableMainMemoryRoutingDiagnostics {
  return Boolean(
    (diagnostics as MutableMainMemoryRoutingDiagnostics | undefined)?.[
      MAIN_MEMORY_ROUTING_CAPTURED
    ],
  );
}

function markMainMemoryRoutingDiagnosticsCaptured(
  diagnostics: MainMemoryRoutingDiagnostics | undefined,
): void {
  if (!diagnostics) {
    return;
  }
  (diagnostics as MutableMainMemoryRoutingDiagnostics)[MAIN_MEMORY_ROUTING_CAPTURED] = true;
}
...
export function createMainMemoryToolChoiceWrapper(
  ...
  params: {
    ...
    version?: string;
    commit?: string | null;
    diagnostics?: MainMemoryRoutingDiagnostics;
  },
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    if (hasCapturedMainMemoryRoutingDiagnostics(params.diagnostics)) {
      return underlying(model, context, options);
    }
    const decision = resolveMainMemoryRoutingDecisionBase({
    ...
    updateMainMemoryRoutingDiagnostics(params.diagnostics, decision);
    if (decision.selectedTarget === "none") {
      updateMainMemoryRoutingDiagnostics(params.diagnostics, {
        applicationReasonCode: "no_selected_target",
      });
      return underlying(model, context, options);
    }
    return streamWithPayloadPatch(underlying, model, context, options, (payloadObj) => {
      const existingChoiceRaw = payloadObj.tool_choice;
      const existingChoice = normalizeToolChoiceSnapshot(existingChoiceRaw);
      updateMainMemoryRoutingDiagnostics(params.diagnostics, {
        toolChoiceBeforePatch: existingChoice,
      });
      if (existingChoiceRaw !== undefined && existingChoiceRaw !== "auto") {
        updateMainMemoryRoutingDiagnostics(params.diagnostics, {
          applicationReasonCode: "existing_tool_choice_preserved",
          toolChoiceAfterPatch: existingChoice,
        });
        return;
      }
      log.debug(`pinning tool_choice=${decision.selectedTarget} for main memory-informed prompt`);
      payloadObj.tool_choice = {
        type: "function",
        name: decision.selectedTarget,
      };
      updateMainMemoryRoutingDiagnostics(params.diagnostics, {
        applicationReasonCode: "pinned_selected_target",
        toolChoiceAfterPatch: normalizeToolChoiceSnapshot(payloadObj.tool_choice),
      });
    });
  };
}

export function createMainMemoryRoutingDiagnosticsWrapper(
  baseStreamFn: StreamFn | undefined,
  params: { diagnostics?: MainMemoryRoutingDiagnostics },
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    if (hasCapturedMainMemoryRoutingDiagnostics(params.diagnostics)) {
    ...
    return streamWithPayloadPatch(underlying, model, context, options, (payloadObj) => {
      const finalToolChoice = normalizeToolChoiceSnapshot(payloadObj.tool_choice);
      const afterPatch = params.diagnostics?.toolChoiceAfterPatch;
      updateMainMemoryRoutingDiagnostics(params.diagnostics, {
        finalToolChoice,
        finalToolChoiceChanged:
          JSON.stringify(finalToolChoice) !== JSON.stringify(afterPatch ?? null),
      });
      markMainMemoryRoutingDiagnosticsCaptured(params.diagnostics);
    });
  };
}
