...
async function persistSubagentAbortedLastRun(params: {
  ...
  try {
    await subagentControlDeps.updateSessionStore(params.storePath, (store) => {
      const current = store[params.childSessionKey];
      if (!current) {
        return;
      }
      current.abortedLastRun = params.abortedLastRun;
      current.updatedAt = Date.now();
      store[params.childSessionKey] = current;
    });
  } catch (error) {
    logVerbose(
...
export async function killSubagentRunAdmin(params: { cfg: OpenClawConfig; sessionKey: string }) {
  ...
  return {
    ...
    alreadyEnded: Boolean(entry.endedAt),
    cascadeKilled: cascade.killed,
    cascadeLabels: cascade.killed > 0 ? cascade.labels : undefined,
  };
}
