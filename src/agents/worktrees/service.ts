...
const DEFAULT_WORKTREE_SETUP_PATH = "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin";
...
async function requireSystemChangeBase(params: {
  ...
  const resolvedBase = await runGit(params.repository.repoRoot, [
    "rev-parse",
    "--verify",
    "--end-of-options",
    `${baseRef}^{commit}`,
...
export class ManagedWorktreeService {
  ...
  async create(params: CreateManagedWorktreeParams): Promise<ManagedWorktreeRecord> {
    ...
    const base = systemChangeBase
      ? { base: systemChangeBase, remote: false }
      : await resolveBase(repository.repoRoot, params.baseRef);
