...
import { buildAttemptPromptRuntime } from "../attempt-prompt-runtime.js";
import { buildAttemptToolRuntime } from "../attempt-tool-runtime.js";
import {
...
import {
  buildAttemptContextRuntimeContext,
  finalizeAttemptContextManagerTurn,
  runBeforeSubmitContextPressure,
...
} from "../pi-embedded-runner/cache-ttl.js";
...
import { prepareGooglePromptCacheStreamFn } from "../pi-embedded-runner/google-prompt-cache.js";
import {
...
} from "../pi-embedded-runner/prompt-cache-observability.js";
import { resolveCacheRetention } from "../pi-embedded-runner/prompt-cache-retention.js";
import {
...
} from "../pi-embedded-runner/run/attempt.sessions-yield.js";
import { wrapStreamFnHandleSensitiveStopReason } from "../pi-embedded-runner/run/attempt.stop-reason-recovery.js";
import {
  buildEmbeddedSubscriptionParams,
  cleanupEmbeddedAttemptResources,
} from "../pi-embedded-runner/run/attempt.subscription-cleanup.js";
import {
  appendAttemptCacheTtlIfNeeded,
  composeSystemPromptWithHookContext,
  shouldPersistCompletedBootstrapTurn,
  shouldUseOpenAIWebSocketTransport,
} from "../pi-embedded-runner/run/attempt.thread-helpers.js";
import {
  shouldRepairMalformedAnthropicToolCallArguments,
  wrapStreamFnDecodeXaiToolCallArguments,
  wrapStreamFnRepairMalformedToolCallArguments,
} from "../pi-embedded-runner/run/attempt.tool-call-argument-repair.js";
import {
  sanitizeReplayToolCallIdsForStream,
  wrapStreamFnSanitizeMalformedToolCalls,
  wrapStreamFnTrimToolCallNames,
} from "../pi-embedded-runner/run/attempt.tool-call-normalization.js";
import { waitForCompactionRetryWithAggregateTimeout } from "../pi-embedded-runner/run/compaction-retry-aggregate-timeout.js";
import {
  resolveRunTimeoutDuringCompaction,
  selectCompactionTimeoutSnapshot,
  shouldFlagCompactionTimeout,
} from "../pi-embedded-runner/run/compaction-timeout.js";
import { pruneProcessedHistoryImages } from "../pi-embedded-runner/run/history-image-prune.js";
...
} from "../pi-embedded-runner/run/llm-idle-timeout.js";
import { createProgressLeaseTimeout } from "../pi-embedded-runner/run/progress-lease-timeout.js";
import type {
...
} from "../pi-embedded-runner/stream-resolution.js";
...
} from "../pi-embedded-runner/tool-result-context-guard.js";
import {
...
import {
  acquireAttemptSessionLockRuntime,
  openAttemptSessionTranscriptRuntime,
  type AttemptSessionTranscriptRuntime,
} from "../session-runtime/attempt-session.js";
...
export { filterEffectiveToolsForNodeAgentNativeTaskMode } from "../attempt-tool-runtime.js";
  ...
  return (model, context, options) => {
    return streamWithPayloadPatch(params.streamFn, model, context, options, (payload) => {
      ...
      if (params.onAgentEvent) {
        void Promise.resolve(params.onAgentEvent({ stream: "node-agent", data: event })).catch(
          (error) => {
            log.warn(`provider request diagnostics event dispatch failed: ${String(error)}`);
          },
        );
      }
    });
...
function appendProviderWaitLockHandoffEvent(params: {
  ...
  if (params.onAgentEvent) {
    void Promise.resolve(params.onAgentEvent({ stream: "node-agent", data: event })).catch(
      (error) => {
        log.warn(`provider wait lock handoff event dispatch failed: ${String(error)}`);
      },
    );
  }
}
...
export async function runInteractionAttempt(
  ...
  try {
    ...
    const { parentSessionLockHandoff } = await acquireAttemptSessionLockRuntime(params);
    ...
    const nodeAgentNativeTaskMode =
      params.nodeAgentNativeTaskMode?.enabled === true && parentSessionLockHandoff.runChildTask
        ...
        : params.nodeAgentNativeTaskMode;
    const toolRuntime = await buildAttemptToolRuntime({
      attempt: params,
      sessionAgentId,
      sandboxSessionKey,
      agentDir,
    ...
    const promptRuntime = await buildAttemptPromptRuntime({
      attempt: params,
      sessionAgentId,
      defaultAgentId,
      effectiveWorkspace,
    ...
    const { buildAttemptSystemPromptReport, promptProfile, effectivePromptMode, heartbeatPrompt } =
      promptRuntime;
    let { systemPromptReport, systemPromptText } = promptRuntime;

    let sessionManager: AttemptSessionTranscriptRuntime["sessionManager"] | undefined;
    let session: Awaited<ReturnType<typeof createAgentSession>>["session"] | undefined;
    let removeToolResultContextGuard: (() => void) | undefined;
    try {
      const sessionRuntime = await openAttemptSessionTranscriptRuntime({
        attempt: params,
        sessionAgentId,
        effectiveWorkspace,
        agentDir,
        allowedToolNames,
        runtimeContext: buildAttemptContextRuntimeContext({
          attempt: params,
          workspaceDir: effectiveWorkspace,
          agentDir,
      ...
      const { transcriptPolicy } = sessionRuntime;
      ...
      try {
        ...
        try {
          ...
          const beforeSubmitContextPressure = await runBeforeSubmitContextPressure({
            messages: activeSession.messages,
            systemPrompt: systemPromptText,
            prompt: effectivePrompt,
            reserveTokens,
            ...
            agentId: sessionAgentId,
            runId: params.runId,
            nativeTaskResultAwaitingParentContext: hasDeliveredNativeTaskResultAwaitingParentTurn(
              activeSession.messages,
            ),
            nativeTaskResultAwaitingParentContextReason:
              NATIVE_TASK_RESULT_AWAITING_PARENT_CONTEXT_REASON,
            nativeTaskResultAwaitingParentContextErrorText:
              NATIVE_TASK_RESULT_AWAITING_PARENT_CONTEXT_ERROR_TEXT,
            nodeAgentSessionTraceEvents,
            log,
          });
          if (beforeSubmitContextPressure.outcome) {
            contextPressureOutcome = beforeSubmitContextPressure.outcome;
          }
          if (beforeSubmitContextPressure.promptError) {
            promptError = beforeSubmitContextPressure.promptError;
            promptErrorOrigin = beforeSubmitContextPressure.promptErrorOrigin ?? "precheck";
          }
          if (beforeSubmitContextPressure.skipPromptSubmission) {
            skipPromptSubmission = true;
          }
          ...
          if (params.onAgentEvent) {
            void Promise.resolve(
              ...
              log.warn(`session launch event dispatch failed: ${String(error)}`);
            });
          }
        ...
        await finalizeAttemptContextManagerTurn({
          attempt: params,
          effectiveWorkspace,
          agentDir,
          currentTokenCount: derivePromptTokens(lastCallUsage),
          promptCache,
