      ...
      Type.Object(
        {
          ...
          executionWorkspace: Type.Optional(
            Type.Object(
              {
                type: Type.Literal("loaded-source"),
                access: Type.Union([Type.Literal("inspect"), Type.Literal("modify")]),
              },
  ...
  return {
    ...
    execute: async () => {
      ...
      const agents: AgentListEntry[] = ordered.map((id) => {
        ...
        if (agentConfig?.executionWorkspace) {
          entry.executionWorkspace = agentConfig.executionWorkspace;
        }
        return entry;
      });
