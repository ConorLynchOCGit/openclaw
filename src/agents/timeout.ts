...
export const MAX_SAFE_AGENT_TIMEOUT_MS = 2_147_000_000;
...
export function resolveAgentTimeoutMs(opts: {
  ...
  const clampTimeoutMs = (valueMs: number) =>
    Math.min(Math.max(valueMs, minMs), MAX_SAFE_AGENT_TIMEOUT_MS);
  ...
  const NO_TIMEOUT_MS = MAX_SAFE_AGENT_TIMEOUT_MS;
