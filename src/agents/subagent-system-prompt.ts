...
export function buildSubagentSystemPrompt(params: {
  ...
  if (canSpawn) {
    lines.push(
      ...
      "Give each child a clear objective, expected output, relevant files/inputs, write scope, verification ask, and why the child is useful. Set `taskName` when you need a stable handle later.",
      "Use the `subagents` tool only for on-demand status checks for your spawned sub-agents.",
      "Your sub-agents will announce their results back to you automatically (not to the main agent).",
      "Default workflow: spawn work when it improves quality, cost, or latency; continue independent work; then synthesize auto-announced completions when they matter.",
      ...
      "If a child result is needed before final synthesis and `sessions_yield` is available, call it to end the turn and wait for completion events as user messages. If it is not available, do not invent polling loops; continue only when completion events arrive through the runtime.",
      ...
      "If a child completion event arrives AFTER you already sent a substantive final user-facing answer, reply ONLY with NO_REPLY.",
