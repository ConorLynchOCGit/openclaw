...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "5. **Trust push-based completion** - Descendant results are auto-announced back to you as session context. If `sessions_yield` is available, use it when you need to wait; if one child returns but more evidence is still needed, it is valid to yield again. Do not busy-poll for status.",
  ...
  if (canSpawn) {
    lines.push(
      ...
      "Give each child a clear objective, useful output shape, relevant files/inputs, write scope, verification ask, and why the child is useful. Set `taskName` when you need a stable handle later.",
      "Use the `subagents` tool only for on-demand status checks for your spawned sub-agents.",
      "Your sub-agents will announce their results back to you automatically (not to the main agent).",
      "Spawn work when it improves quality, cost, or latency; continue independent work; then use auto-announced completions when they matter.",
      ...
      "If you want runtime-resumed child output and `sessions_yield` is available, call it to end the turn and wait for completion events as session context. If a partial completion arrives but the assigned task still needs more child results, synthesize what changed and yield again. If `sessions_yield` is not available, do not invent polling loops; continue only when completion events arrive through the runtime.",
      "Do not force every task through a fixed route or static DAG; decide dynamically whether child output is necessary before final synthesis.",
      "Do NOT repeatedly poll `subagents list` in a loop unless you are actively checking visibility/debugging.",
      "Coordinate child work when useful, but keep the assigned task and your own judgment in control.",
      ...
      ...(acpEnabled
        ? [
            ...
            "Avoid polling loops; spawn and use results when they matter.",
          ]
        : []),
      "",
    );
