...
import { usageSnapshotFromNormalizedUsage } from "../context-engine/pressure/index.js";
...
import {
  resolveAutoCompactionCustomInstructions,
  runActualUsageContextPressure,
  runOverflowToolResultFallbackTruncation,
  runProviderOverflowContextPressure,
  runTimeoutHighUsageContextPressure,
} from "./context-manager-runtime.js";
...
import type { ApiKeyInfo } from "./model-auth-runtime.js";
...
import { handleAssistantFailover } from "./pi-embedded-runner/run/assistant-failover.js";
import { createEmbeddedRunAuthController } from "./pi-embedded-runner/run/auth-controller.js";
import { createFailoverDecisionLogger } from "./pi-embedded-runner/run/failover-observation.js";
import {
  mergeRetryFailoverReason,
  resolveRunFailoverDecision,
} from "./pi-embedded-runner/run/failover-policy.js";
import {
  buildErrorAgentMeta,
...
} from "./pi-embedded-runner/run/helpers.js";
import {
...
} from "./pi-embedded-runner/run/incomplete-turn.js";
import { buildEmbeddedRunPayloads } from "./pi-embedded-runner/run/payloads.js";
import { handleRetryLimitExhaustion } from "./pi-embedded-runner/run/retry-limit.js";
import { mergeAttemptToolMediaPayloads } from "./pi-embedded-runner/run/tool-media-payloads.js";
import type {
...
} from "./pi-embedded-runner/usage-accumulator.js";
import { derivePromptTokens, normalizeUsage, type UsageLike } from "./usage.js";

const MAX_SAME_MODEL_IDLE_TIMEOUT_RETRIES = 1;

type InteractionRuntimeParams = AgentRuntimeInvocation["runtime"];
...
function emitEmbeddedRunStartTiming(
  ...
  logEmbeddedRunStartTiming(label, startedAt, details);
...
export async function runInteractionRuntime(
  input: AgentInteractionRuntimeInput,
): Promise<EmbeddedPiRunResult> {
  const params = input.invocation.runtime;
  return await withExternalCliAuthSyncSuppressed(async () =>
    runEmbeddedPiAgentWithExternalCliAuthSyncSuppressed(params, input.runtimeContext),
  );
}

export type AgentInteractionRuntimeInput = {
  turn?: AgentTurn;
...
async function runEmbeddedPiAgentWithExternalCliAuthSyncSuppressed(
  ...
  const emitStartTiming = (label: string, details: Record<string, unknown> = {}): void =>
    emitEmbeddedRunStartTiming(params, label, runStartedAt, details);
  emitStartTiming("caller_owned_scheduler_entered", {
    schedulingMode: "caller_owned",
  });
  throwIfAborted();
  throwIfAborted();
  const started = runStartedAt;
  emitStartTiming("interaction_runtime_entered", {
    sessionId: params.sessionId,
    sessionKey: params.sessionKey,
    agentId: params.agentId,
    schedulingMode: "caller_owned",
  });
  const { runEnvironment } = runtimeContext;
  const workspaceResolution = runEnvironment.workspaceResolution;
  const resolvedWorkspace = runEnvironment.resolvedWorkspace;
  let provider = runEnvironment.provider;
  let modelId = runEnvironment.modelId;
  const agentDir = runEnvironment.agentDir;
  const normalizedSessionKey = runEnvironment.normalizedSessionKey ?? undefined;
  const fallbackConfigured = runEnvironment.fallbackConfigured;
  emitStartTiming("agent_runtime_core_services_applied", {
    requestShape: runtimeContext.requestShape,
    promptProfile: runtimeContext.prompt.profile,
    toolPolicyVisibleCount: runtimeContext.toolRuntime.visibleToolNames?.length ?? null,
    toolPolicyRequiredCount: runtimeContext.toolRuntime.requiredToolNames?.length ?? null,
    transcriptRoot: runtimeContext.workspace.transcriptRoot,
    artifactRoot: runtimeContext.workspace.artifactRoot,
    ...
    contextPressureOwner: runtimeContext.contextPressure?.owner ?? null,
    modelAuthOwner: runtimeContext.modelAuth.owner,
    providerCapabilityOwner: runtimeContext.providerCapability.owner,
    runtimeEventSinkOwner: runtimeContext.runtimeEventSink.owner,
    receiptCount: runtimeContext.receipts.length,
  });
  const resolvedSessionKey = normalizedSessionKey;
  const hookRunner = getGlobalHookRunner();
  const modelAuthRuntime = runtimeContext.modelAuthRuntime;
  provider = modelAuthRuntime.provider;
  modelId = modelAuthRuntime.modelId;
  const legacyBeforeAgentStartResult = modelAuthRuntime.legacyBeforeAgentStartResult;
  const authStorage = modelAuthRuntime.authStorage;
  const modelRegistry = modelAuthRuntime.modelRegistry;
  let runtimeModel = modelAuthRuntime.runtimeModel;
  const ctxInfo = modelAuthRuntime.ctxInfo;
  let effectiveModel = modelAuthRuntime.effectiveModel;
  const authStore = modelAuthRuntime.authStore;
  const preferredProfileId = modelAuthRuntime.preferredProfileId;
  const lockedProfileId = modelAuthRuntime.lockedProfileId;
  const profileCandidates = modelAuthRuntime.profileCandidates;
  emitStartTiming("model_auth_runtime_applied", {
    provider,
    modelId,
  ...
  emitStartTiming("model_auth_profile_initialized", {
    provider,
    modelId,
  });
  ...
  const runtimeJobEnvelopeOwnsPostTurnReduction =
    params.source === "runtime_generation" &&
    (params.nativeExecutionSession?.enabled === true ||
      params.nativeRuntimeTools?.some((tool) => tool.name === "node_finish") === true);
  ...
  const maybeMarkAuthProfileFailure = async (failure: {
    ...
    config?: InteractionRuntimeParams["config"];
    agentDir?: InteractionRuntimeParams["agentDir"];
    modelId?: string;
  }) => {
  ...
  const contextPressureContext = runtimeContext.contextPressure;
  if (!contextPressureContext) {
    throw new Error("InteractionRuntime requires AgentRuntimeCore context pressure runtime.");
  }
  const contextRuntime = contextPressureContext.runtime;
  ...
  emitStartTiming("context_runtime_resolved", {
    ownsCompaction: contextEngine.info.ownsCompaction === true,
    owner: contextPressureContext.owner,
  });
  const providerRunTurn = runtimeContext.providerCapability.runTurn;
  if (typeof providerRunTurn !== "function") {
    throw new Error("InteractionRuntime requires AgentRuntimeCore provider capability.");
  }
  const runAttemptWithProviderCapability = async (
    attemptParams: EmbeddedRunAttemptParams,
  ): Promise<EmbeddedRunAttemptResult> =>
    (await providerRunTurn({
      attempt: attemptParams,
      signal: params.abortSignal,
      emit: (event) => {
        emitStartTiming(event.phase, {
          provider: event.provider,
          modelId: event.model,
          providerTransportKind: event.transportKind,
  ...
  try {
    ...
    while (true) {
      ...
      emitStartTiming("attempt_workspace_ready", {
        runLoopIterations,
        workspaceDir: resolvedWorkspace,
      });
      ...
      try {
        const authLease = await recheckProviderAuthLease();
        emitStartTiming("provider_auth_rechecked", {
          provider: authLease.provider,
          modelId: authLease.modelId,
          authMode: authLease.authMode,
          credentialSourceClass: authLease.credentialSourceClass,
          profileId: authLease.profileId,
          runtimeAuth: authLease.runtimeAuth,
          expiresInMs: authLease.expiresInMs,
          checkedAt: authLease.checkedAt,
        });
      } catch (err) {
        emitStartTiming("provider_auth_recheck_failed", {
          provider,
          modelId,
      ...
      emitStartTiming("provider_turn_entering", {
        provider,
        modelId,
        thinkLevel,
        reasoningLevel: params.reasoningLevel,
      });
      const attempt = await runAttemptWithProviderCapability({
        sessionId: params.sessionId,
        sessionKey: resolvedSessionKey,
        trigger: params.trigger,
      ...
      if (timedOut && !timedOutDuringCompaction) {
        ...
        if (timeoutCompactionAttempts >= MAX_TIMEOUT_COMPACTION_ATTEMPTS) {
        ...
        } else if (typeof lastTurnPromptTokens === "number" && tokenUsedRatio > 0.65) {
          ...
          const timeoutRecovery = await runTimeoutHighUsageContextPressure({
            runParams: params,
            ...
            attemptNumber: timeoutCompactionAttempts,
          ...
          autoCompactionCount += timeoutRecovery.compactionCountIncrement;
          if (timeoutRecovery.shouldRetryPrompt) {
            continue;
          }
        }
      }
      ...
      if (contextOverflowError) {
        ...
        if (
          ...
          const overflowRecovery = await runProviderOverflowContextPressure({
            runParams: params,
            attemptResult: attempt,
            contextPressure,
            contextPressureOutcome,
            ...
            attemptNumber: overflowCompactionAttempts,
          ...
          autoCompactionCount += overflowRecovery.compactionCountIncrement;
          toolResultTruncationAttempted = overflowRecovery.toolResultTruncationAttempted;
          if (overflowRecovery.shouldRetryPrompt) {
            continue;
          }
        }
        if (!providerOverflowRecoveryAttempted) {
          const fallbackTruncation = await runOverflowToolResultFallbackTruncation({
            runParams: params,
            attemptResult: attempt,
            provider,
            modelId,
          ...
          toolResultTruncationAttempted = fallbackTruncation.toolResultTruncationAttempted;
          if (fallbackTruncation.shouldRetryPrompt) {
            continue;
          }
        }
        if (
      ...
      if (actualUsage && !promptError && !aborted && !timedOut) {
        ...
        autoCompactionCount += actualUsageContextPressure.compactionCountIncrement;
      }
      ...
      const agentMeta: EmbeddedPiAgentMeta = {
        ...
        model: sessionLastAssistant?.model ?? runtimeModel.id,
      ...
      if (
        ...
        emitStartTiming("runtime_job_envelope_reduction_requested", {
          provider: activeErrorContext.provider,
          modelId: activeErrorContext.model,
          payloadCount,
          planningOnlyRetryCandidate: Boolean(nextPlanningOnlyRetryInstruction),
          reasoningOnlyRetryCandidate: Boolean(nextReasoningOnlyRetryInstruction),
          emptyResponseRetryCandidate: Boolean(nextEmptyResponseRetryInstruction),
        });
      }
      if (
        !runtimeJobEnvelopeOwnsPostTurnReduction &&
      ...
      return {
        ...
        meta: {
          ...
          executionTrace: {
            ...
            winnerModel: sessionLastAssistant?.model ?? runtimeModel.id,
            attempts:
