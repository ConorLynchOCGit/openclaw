...
export async function statusCommand(
  ...
  const modelMemoryValue = (() => {
    if (!modelMemoryRuntime.enabled) {
      const reason = modelMemoryRuntime.reason ? ` (${modelMemoryRuntime.reason})` : "";
      return muted(`disabled${reason}`);
    }
    const parts = ["enabled"];
    if (!modelMemoryRuntime.databaseConfigured) {
      parts.push(
        warn(
          `db unavailable${modelMemoryRuntime.databaseError ? ` (${modelMemoryRuntime.databaseError})` : ""}`,
        ),
      );
      return parts.join(" · ");
    }
    if (modelMemoryRuntime.databaseName) {
      parts.push(`db ${modelMemoryRuntime.databaseName}`);
    }
    parts.push(modelMemoryRuntime.captureWritesEnabled ? ok("capture on") : muted("capture off"));
    parts.push(
      modelMemoryRuntime.contextInjectionEnabled ? ok("context on") : muted("context off"),
    );
    parts.push(
      modelMemoryRuntime.legacyMemorySlotDisabled ? ok("legacy slot off") : warn("legacy slot on"),
    );
    parts.push(
      modelMemoryRuntime.legacyMemorySearchDisabled
        ? ok("legacy search off")
        : warn("legacy search on"),
    );
    if (modelMemoryRuntime.includeRetrievalPacks) {
      parts.push("retrieval packs on");
    }
    return parts.join(" · ");
  })();
  ...
  const overviewRows = [
    ...
    { Item: "Model memory", Value: modelMemoryValue },
    { Item: "Legacy memory", Value: memoryValue },
