...
/** Builds the exact compact state used to authorize a same-owner correction episode. */
...
/** Selects the newest active managed flow without changing generic latest-flow lookup behavior. */
...
/** Selects the newest terminal managed flow for compact post-closeout continuity only. */
...
/**
 * Verifies an exact closeout handoff before the same logical owner creates a linked correction
 * flow, including after a native session reset rotates its physical model context.
 */
