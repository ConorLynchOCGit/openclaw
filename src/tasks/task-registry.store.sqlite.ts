...
type TaskDeliveryStateRow = {
  ...
  last_replayed_event_at: number | bigint | null;
};
...
function rowToTaskDeliveryState(row: TaskDeliveryStateRow): TaskDeliveryState {
  ...
  const lastReplayedEventAt = normalizeNumber(row.last_replayed_event_at);
  return {
    taskId: row.task_id,
    ...(requesterOrigin ? { requesterOrigin } : {}),
    ...(lastNotifiedEventAt != null ? { lastNotifiedEventAt } : {}),
    ...(lastReplayedEventAt != null ? { lastReplayedEventAt } : {}),
  };
}
...
function bindTaskDeliveryState(state: TaskDeliveryState) {
  return {
    ...
    last_replayed_event_at: state.lastReplayedEventAt ?? null,
  };
}
...
function ensureSchema(db: DatabaseSync) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS task_delivery_state (
      ...
      last_replayed_event_at INTEGER
    );
  `);
  const deliveryRows = db.prepare(`PRAGMA table_info(task_delivery_state)`).all() as TableInfoRow[];
  if (!deliveryRows.some((row) => row.name === "last_replayed_event_at")) {
    db.exec(`ALTER TABLE task_delivery_state ADD COLUMN last_replayed_event_at INTEGER;`);
  }
