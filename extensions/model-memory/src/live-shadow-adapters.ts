...
function toRuntimeObservation(input: {
  ...
  writeResults: Array<{
    decision: "ignore" | "dedupe" | "write" | "supersede";
  ...
  return {
    ...
    writeObservations: input.writeResults.map((entry, index) => {
      const capturedObject = input.capturedObjects[index];
      if (!capturedObject) {
        throw new Error(`missing captured object for write result ${index}`);
      }
      return {
        decision: entry.decision,
        identityKey:
          entry.memoryObject?.identityKey ??
          deriveMemoryIdentity(capturedObject.object).identityKey,
