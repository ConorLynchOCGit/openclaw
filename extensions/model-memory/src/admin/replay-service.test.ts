...
function createReplayPreferenceInterpreter(params: {
...
describe("replay-service", () => {
  it("replays controlled captures through the database-backed service path", async () => {
    ...
    const previousLegacyFallback =
      process.env.MODEL_MEMORY_LEGACY_CAPTURED_OBJECT_WRITE_FALLBACK_ENABLED;
    process.env.MODEL_MEMORY_DOCUMENT_INGEST_ENGINE = "v1";
    process.env.MODEL_MEMORY_LEGACY_CAPTURED_OBJECT_WRITE_FALLBACK_ENABLED = "true";
    try {
      ...
      if (previousLegacyFallback === undefined) {
        delete process.env.MODEL_MEMORY_LEGACY_CAPTURED_OBJECT_WRITE_FALLBACK_ENABLED;
      } else {
        process.env.MODEL_MEMORY_LEGACY_CAPTURED_OBJECT_WRITE_FALLBACK_ENABLED =
          previousLegacyFallback;
      }
  ...
  it("propagates runtime-state env so replayed captures emit closeout artifacts", async () => {
    const database = await createPgMemTestDatabase();
    const stateDir = await mkdtemp(path.join(os.tmpdir(), "model-memory-replay-closeout-"));
    const previousEngine = process.env.MODEL_MEMORY_DOCUMENT_INGEST_ENGINE;
    process.env.MODEL_MEMORY_DOCUMENT_INGEST_ENGINE = "v1";
    try {
      await applyModelMemoryMigrations(database.sql);
      ...
      const result = await replay.replayOrdinaryTurn({
        turn: {
          currentTurnText:
            "Standing preference: include exact artifact paths in replay validation.",
          sessionId: "session-replay-closeout",
        },
        modelId: "model-doc-001",
        candidateModelId: "model-doc-001",
        interpreter: createReplayPreferenceInterpreter({
          evidenceQuote: "Standing preference: include exact artifact paths in replay validation.",
          canonicalText: "The user prefers exact artifact paths in replay validation.",
          searchText: "user prefers exact artifact paths replay validation",
          object: "exact artifact paths in replay validation",
        }),
      });
