...
import type { CompactionPlanInput, CompactionPlanResult } from "../db/runtime.js";
...
type MemoryCompactionPlanRawParams = ToolRawParams;

const MemoryCompactionPlanToolSchema = Type.Object(
  {
...
export function normalizeMemoryCompactionPlanInput(params: {
  rawParams: MemoryCompactionPlanRawParams;
...
export async function planMemoryCompactionFromTool(params: {
  runtime: MemoryMiddlewareRuntime;
  input: CompactionPlanInput;
}): Promise<CompactionPlanResult> {
  return params.runtime.compactionPlanning.plan(params.input);
}

export function createMemoryCompactionPlanTool(params: {
  runtime: MemoryMiddlewareRuntime;
  context?: OpenClawPluginToolContext;
}): AnyAgentTool {
  return {
    name: "memory_compaction_plan",
    label: "Memory Compaction Plan",
    description:
      "Plan the next bounded context-pressure response for a session using microcompaction signals, session memory, and prompt-pressure hints.",
    parameters: MemoryCompactionPlanToolSchema,
    async execute(_toolCallId: string, rawParams: MemoryCompactionPlanRawParams) {
      const input = normalizeMemoryCompactionPlanInput({
        rawParams,
        context: params.context,
      });
      const result = await planMemoryCompactionFromTool({
