...
function buildCodexNativeCodingTeamInstruction(
  ...
  const lines = [
    ...
    "Prefer `rg` and bounded file reads for source inspection. Batch independent searches, reads, and validations through the native Codex parallel tool-call form available in this thread. In app-server event streams this may appear as several concrete tool calls in one model step rather than a visible `multi_tool_use.parallel` wrapper.",
    ...
    "Prefer direct project custom-agent names such as `project_explorer`, `test_engineer`, `code_reviewer`, or `codex_reviewer` when the live Codex spawn surface accepts them.",
    "If direct project custom-agent names are not accepted by the live Codex spawn surface, use the Codex built-in `explorer` for read-heavy exploration and `worker` for review, validation, docs research, or implementation support; put `Role: <project role>` and the relevant role contract in the child message.",
    "A failed direct custom-agent spawn does not prove that Codex subagents are unavailable if built-in `explorer` or `worker` still works. Retry once through the matching built-in Codex agent with the project role embedded in the prompt, and record the direct-spawn error in the closeout.",
