...
describe("OpenClaw package-owned Codex repo workbench", () => {
  ...
  it("exposes bounded read-only repo helpers without mutating source", async () => {
    ...
    const readTarget =
      "extensions/codex/system-profile/project/.codex/agents/implementation_planner.toml";
    const readPath = path.join(REPO_ROOT, readTarget);
    ...
    const read = await workbench.repoReadMany(
      { files: [{ path: readTarget, endLine: 8 }] },
      WORKBENCH_OPTIONS,
    );
    ...
    expect(read.results[0]?.text).toContain("1: name =");
