...
export type PostResourceImplementationTaskCompileStatus =
  | "accepted"
  | "split_required"
  | "resource_repair_required"
  | "needs_review";

export type PostResourceImplementationTaskCompileResult = {
  status: PostResourceImplementationTaskCompileStatus;
...
export type PostResourceImplementationTaskCompilerInput = {
  runtimeJobId: string;
  workflowId: string;
  graphId: string;
  nodeId: string;
...
export function compilePostResourceImplementationTaskPackets(
  input: PostResourceImplementationTaskCompilerInput,
): PostResourceImplementationTaskCompileResult {
  const reasonCodes = ["post_resource_implementation_task_compiler_used"];
  ...
  const domainResourceSelectionRefs = unique(input.domainResourceSelectionRefs, 24);
  ...
  if (targetCommitmentIds.length === 0) {
    ...
    reasonCodes.push("post_resource_task_commitment_mapping_missing");
  }
  if (executionIntent === "unspecified") {
    missingFields.push("executionIntent");
    reasonCodes.push("post_resource_task_execution_intent_missing");
  }
  if (executionIntent !== "unspecified" && executionIntent !== "source_edit") {
    missingFields.push("executionIntent");
    reasonCodes.push("post_resource_task_execution_intent_not_edit_required");
  }
  if (!evidenceMode.includes("changed_file_evidence")) {
    missingFields.push("evidenceMode.changed_file_evidence");
    reasonCodes.push("post_resource_task_changed_file_evidence_mode_missing");
  }
  if (contextPacketRefs.length === 0) {
    reasonCodes.push("post_resource_task_node_local_resource_window_required");
  }
  if (executableFileRefs.length === 0) {
    missingFields.push("readableFileRefs");
    reasonCodes.push("post_resource_task_readable_file_snapshots_missing");
  }
  if (missingFileRefs.length > 0) {
    reasonCodes.push("post_resource_task_missing_target_refs_block_execution");
  }
  if (validationCommandRefs.length === 0 && validationDiscoveryPlan.length === 0) {
    ...
    reasonCodes.push("post_resource_task_validation_plan_missing");
  }
  if (acceptanceCriteria.length === 0) {
    missingFields.push("acceptanceCriteria");
    reasonCodes.push("post_resource_task_acceptance_criteria_missing");
  }
  if (multiFileOrMultiCommitment && fileChangeIntents.length === 0) {
    if (newFileRefs.length === 0) {
      missingFields.push("fileChangeIntents");
      reasonCodes.push("post_resource_task_semantic_microtask_refinement_required");
    }
  }
  if (executableFileRefs.length > 1) {
    ...
    if (uncoveredTargetRefs.length > 0) {
      ...
      reasonCodes.push("post_resource_task_file_change_intent_coverage_missing");
    }
  }
  ...
  if (chunks.length > 1) {
    reasonCodes.push("post_resource_task_split_required_for_file_resolved_microtasks");
  }
  const packets = chunks.map((fileRefs, index) => {
    ...
    return buildImplementationTaskPacket({
      ...
      sourceResourceHandoffRefs: input.sourceResourceHandoffRefs ?? contextPacketRefs,
      ...
      capabilityFit:
        input.capabilityFit ??
        "Selected by post-resource scheduler policy as a concrete file-resolved implementation task.",
  ...
  const invalidPackets = packets
    ...
    .filter((entry) => entry.validation.status === "invalid");
  if (invalidPackets.length > 0) {
    return {
      ...
      reasonCodes: [
        ...reasonCodes,
        "post_resource_task_packet_validation_failed",
        ...unique(
  ...
  const contextPendingPackets = packets
    .map((packet) => ({ packet, validation: validateImplementationTaskPacketForWorker(packet) }))
    .filter((entry) => entry.validation.status === "needs_context");
  if (contextPendingPackets.length > 0) {
    reasonCodes.push(
      "post_resource_task_packets_require_node_local_context_before_write_gate",
      ...unique(
        contextPendingPackets.flatMap((entry) => entry.validation.reasonCodes),
        30,
      ),
    );
  }
